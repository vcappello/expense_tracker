...
- **Back button navigation**: the Back button always navigates back in the navigation history to the view of origin (it never pushes a new route, so the back stack is preserved). Example: Main view → Account management → Edit Account: the first Back returns to Account management, the second Back returns to Main view. The same rule applies after saving or deleting from a create/edit view: the app returns to the previous view (`navigate(-1)`, helper `useNavigateBack` in `src/utils/navigation.ts`) instead of pushing a fixed route, so the back stack is preserved. Example: Main view → Account management → New Account → Save → Back → Account management → Back → Main view (never back to the New Account form).
- **Modal base (shared)**: generic overlay modal component (overlay, title, content, actions area). It closes on backdrop click / ESC, uses `role="dialog"` + `aria-modal`, and its style lives in a shared `Modal.css`. It is the base for `ConfirmModal` and `AlertModal`, so that overlay/CSS/accessibility are not duplicated.
- **ConfirmModal**: critical confirmation popup with two buttons (Annulla / Continua). Used for destructive actions: delete of Account and ExpenseType (cascade with totals) and delete confirmations of Expense/Cashflow.
- **AlertModal**: informational popup with a single OK button, used for operation errors (save / load / delete failed). Validation messages for missing required fields use the Toast component instead of a modal (lighter on mobile).
- **No native dialogs**: the app does not use native `window.confirm`, `window.alert` or `window.prompt`; every dialog is a custom React component (Modal base + ConfirmModal / AlertModal + Toast).
